...
function createArweavePathManifest(
  ...
  if (image2TxId) {
    arweavePathManifest.paths[`image2${image2Type}`] = {
      id: image2TxId,
    }
  }
...
async function getFilePairSize({
  ...
  return await [image, animation, manifest, image2].reduce(
    async (accP, file) => {
...
async function getUpdatedManifest(
  ...
  if (image2Link) {
    manifest.properties.files[1].uri = image2Link
  }
  return manifest
}
...
async function processFiles({
  ...
  let image2DataItem: BundlrTransaction | DataItem
  ...
  let image2ContentType = undefined
  if (filePair.image2) {
    const imageBuffer = await readFile(filePair.image2)
    image2ContentType = getType(filePair.image2)
    if (storageType === StorageType.ArweaveSol) {
      //@ts-ignore
      image2DataItem = bundlr.createTransaction(imageBuffer, {
        tags: imageTags.concat({
          name: 'Content-Type',
          value: imageContentType,
        }),
      })
      await (image2DataItem as unknown as BundlrTransaction).sign()
    } else if (storageType === StorageType.ArweaveBundle) {
      image2DataItem = await getImageDataItem(
        signer,
        imageBuffer,
        imageContentType
      )
  ...
  const image2Link = filePair.image2
    ? `https://arweave.net/${image2DataItem.id}?ext=${path
        .extname(filePair.image2)
        .replace('.', '')}`
    : undefined
  ...
  const arweavePathManifest = createArweavePathManifest(
    ...
    filePair.image2 ? image2DataItem.id : undefined,
    filePair.image2 ? `.${getExtension(image2ContentType)}` : undefined
  )
...
export async function* makeArweaveBundleUploadGenerator(
  ...
  const filePairs = assets.map((asset: AssetKey) => {
    ...
    return {
      ...
      image2: path.join(dirname, `${manifestData.properties.files[1].uri}`),
      animation:
  ...
  if (storageType === StorageType.ArweaveSol) {
    ...
    if (currentBalance.lt(bufferCost)) {
      ...
      log.info(
        `Current balance ${currentBalance.toNumber() / LAMPORTS} is sufficient.`
      )
    }
  }
    ...
    const { cacheKeys, dataItems, arweavePathManifestLinks, updatedManifests } =
      await bundleFilePairs.reduce<Promise<ProcessedBundleFilePairs>>(
        ...
        async function processBundleFilePair(accP, filePair) {
          ...
          if (filePair.image2) {
            acc.dataItems.push(image2DataItem as DataItem)
          }
  ...
  if (balance.minus(5000).lte(0)) {
    ...
    try {
      ...
      if (withdrawResponse.status == 200) {
        log.info(
          `Successfully withdrew ${withdrawResponse.data.final / LAMPORTS} SOL.`
        )
