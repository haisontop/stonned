...
export async function createNft({
  ...
  return { mintPub: mint.publicKey, tx }
}

export async function createNftInstr({
  walletKeypair,
  metadataLink,
  metadata,
  mutableMetadata = true,
  ...
  instructions.push(
    SystemProgram.createAccount({
      fromPubkey: mintTo ?? wallet.publicKey,
      newAccountPubkey: mint.publicKey,
      lamports: mintRent,
      space: MintLayout.span,
      programId: TOKEN_PROGRAM_ID,
  ...
  instructions.push(
    createMetadataInstruction(
      ...
      mint.publicKey,
      wallet.publicKey,
      feePayer ?? mintTo ?? wallet.publicKey,
      wallet.publicKey,
      txnData
    )
  )
...
export const createMetadata = async (
  ...
  if (
    ...
    throw new Error('wrong royalty distribution')
  }
...
export async function updatePrimarySaleHappenedViaToken(
  ...
  const metadataProgramId = TOKEN_METADATA_PROGRAM_ID
