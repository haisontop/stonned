...
export function useStartAwakening(
...
export const useEvolutionStats = () => {
  return useAsync(async () => {
    ...
    return {
      ...
      percentage: ((fetchedAccount.length / livingSacApesCount) * 100).toFixed(
        2
      ),
    }
  }, [connection])
...
export function useAwakeningAccounts() {
  const wallet = useWallet()

  return useQuery(['awakeningAccounts', wallet.publicKey], async () => {
    if (
      !wallet ||
      !wallet.publicKey ||
      !wallet.signAllTransactions ||
    ...
    console.log('in useAwakeningAccounts')
    ...
    console.log({ awakeningAccounts })

    /*  const rawBreedingAccounts = rawBreedingAccountsAll.filter(
      (s) => s.account.authority.toBase58() === wallet.publicKey?.toBase58()
    ) */

    let awakeningAccountsWithNfts = await asyncBatch(
      awakeningAccounts,
      async (awakeningAccount, taskIndex: number, workerIndex: number) => {
        try {
          const nft = await getNftWithMetadataNew(awakeningAccount.account.mint)

          const startDate = new Date(
            awakeningAccount.account.start.toNumber() * 1000
          )
          const endDate = addDays(startDate, 4)

          return { nft, awakeningAccount, startDate, endDate }
        } catch (error) {
          console.log('error in asyncBatch', error)
          throw error
        }
    ...
    awakeningAccountsWithNfts = awakeningAccountsWithNfts.sort((a, b) => {
      if (!a!.nft.name.includes('#') || !b!.nft.name.includes('#')) return 1
      return (
        Number(a!.nft.name.split('#')[1]) - Number(b.nft.name.split('#')[1])
      )
    ...
    return awakeningAccountsWithNfts
  })
}
