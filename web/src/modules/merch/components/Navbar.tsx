...
const MerchNavbar: FC<NavbarProps> = (props: NavbarProps) => {
  const { isOpen, onToggle } = useDisclosure()
  const { status } = useContext(MerchDropContext)

  const fontColor =
    status === MerchDropStatus.WAITING
      ? 'white'
    ...
    >
      <Box display={{ base: 'none', md: 'flex' }}>
        <DesktopNav
          fontColor={fontColor}
          bgColor={bgColor}
          showLogo={status !== MerchDropStatus.WAITING}
          showConnect={status !== MerchDropStatus.WAITING}
        ></DesktopNav>
      </Box>

      <Flex display={{ base: 'flex', md: 'none' }} padding='0 2rem' mt='1rem'>
        <Box flex='1'>
          <IconButton
            onClick={onToggle}
            icon={
              isOpen ? (
                <CloseIcon color={fontColor} w={3} h={3} />
              ) : (
                <HamburgerIcon color={fontColor} w={5} h={5} />
              )
            }
            variant={'ghost'}
        ...
        <Link href='/' justifySelf='flex-end' flex='1' textAlign='right'>
          <Image
...
const MobileNav = ({
  ...
  return (
    <Stack bg={bgColor} color={fontColor} p={4} display={{ md: 'none' }}>
      {NAV_ITEMS.map((navItem) => (
        <Box
          key={navItem.label}
