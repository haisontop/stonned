...
import { getNftsFromOwnerByCreatorsWithoutOfChainMeta } from './splUtils'
...
export async function getSacFamilyNftsCount(
  ownerAddressString: string,
) {
  const stakedNfts = await getStakingAccountsForOwner(ownerAddressString)

  const ownedNfts = await getNftsFromOwnerByCreatorsWithoutOfChainMeta({
    owner: pub(ownerAddressString),
    creators: [pub(sacCollection.creator), pub(nukedCollection.creator)],
  })

  const evolutionNfts = await getEvolutionAccountsForOwner(ownerAddressString)
  ...
  const breedingNftsOfRenters = await getBreedingAccountsOfRenters(
    ownerAddressString
  )
  const nftsInRescuePool = await getRentingAccounts(ownerAddressString)
